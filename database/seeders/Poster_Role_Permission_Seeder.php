...
class Poster_Role_Permission_Seeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if (Role::where("name", "khaosat-poster")->count() == 0) {
            $khaosat_poster = Role::create(['name' => 'khaosat-poster', "description" => "Khảo sát Poster", "group" => "Poster", "order" => 2, "lock" => true,]);
        } else {
            $khaosat_poster = Role::where("name", "khaosat-poster")->first();
        }
